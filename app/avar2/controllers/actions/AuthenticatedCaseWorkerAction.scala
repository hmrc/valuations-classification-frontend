...
@Singleton
class AuthenticatedCaseWorkerAction @Inject()(
  appConfig: AppConfig,
  ...
  override def invokeBlock[A](request: Request[A], block: AuthenticatedCaseWorkerRequest[A] => Future[Result]): Future[Result] = {
    ...
    authorised(predicate).retrieve(
      ...
      case Some(credentials) ~ name ~ email ~ roles =>
        ...
        val caseWorker = CaseWorker(pid, name.flatMap(_.name), email, role)
        block(AuthenticatedCaseWorkerRequest(caseWorker, request))
