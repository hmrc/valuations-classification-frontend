...
class AppConfigSpec extends SpecBase {
  ...
  "Config" should {
    ...
    "Build report url" in {
      realAppConfig.reportAProblemPartialUrl shouldBe "http://localhost:9250/contact/problem_reports_ajax?service=valuations-classification-frontend"
    }

    "Build report non-json url" in {
      realAppConfig.reportAProblemNonJSUrl shouldBe "http://localhost:9250/contact/problem_reports_nonjs?service=valuations-classification-frontend"
    }

    "Build local Binding valuations Base URL" in {
      realAppConfig.bindingvaluationsClassificationUrl shouldBe "http://localhost:9580"
    }
