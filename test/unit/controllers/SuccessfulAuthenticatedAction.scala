...
import connector.{BindingvaluationsClassificationConnector, FakeDataCacheConnector, StrideAuthConnector}
...
class SuccessfulAuthenticatedAction(
  ...
  permissions: Set[Permission] = Set.empty
    ...
    extends AuthenticatedAction(
      ...
      parse         = parse,
      userConnector = mock(classOf[BindingvaluationsClassificationConnector])
    ) {
