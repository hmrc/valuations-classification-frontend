...
class FileStoreConnectorSpec extends ConnectorTest {
    ...
    stubFor(
      post("/file")
        .willReturn(
          aResponse()
            .withStatus(Status.ACCEPTED)
            .withBody(fromResource("filestore/binding-valuations-filestore_upload-response.json"))
        )
    )
  ...
  "Connector download" should {
    "handle missing file" in {
      stubFor(
        get("/digital-valuationss-local/id")
          .willReturn(
            aResponse()
      ...
      val result = await(connector.downloadFile(wireMockUrl + "/digital-valuationss-local/id"))
    ...
    "handle error response" in {
      ...
      assertThrows[Exception](await(connector.downloadFile(wireMockUrl + "/digital-valuationss-local/id")))
    }
  }
