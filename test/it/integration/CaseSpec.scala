...
class CaseSpec extends IntegrationTest with MockitoSugar {
      ...
      stubFor(
        post(urlEqualTo("/file/initiate"))
          .willReturn(
            aResponse()
              .withStatus(OK)
              .withBody(fromResource("filestore/binding-valuations-filestore_initiate-response.json"))
          )
      )
  ...
  "Case Ruling Details" should {
    ...
    "return status SEE_OTHER" in {
      ...
      response.header("Location") should be(Some("/manage-valuations-classifications/cases/v2/1/atar#ruling_tab"))
    }
  ...
  "Case activity details" should {
    ...
    "return status SEE_OTHER" in {
      ...
      // Then
      response.status             shouldBe SEE_OTHER
      response.header("Location") should be(Some("/manage-valuations-classifications/cases/v2/1/atar#activity_tab"))
    }
  ...
  "Case attachments details" should {
    ...
    "return status SEE_OTHER" in {
      ...
      // Then
      response.status             shouldBe SEE_OTHER
      response.header("Location") should be(Some("/manage-valuations-classifications/cases/v2/1/atar#attachments_tab"))
    }
