...
//
//      // When
//      val response: WSResponse = await(requestWithSession(s"http://localhost:$port/manage-valuations-classifications/cases/1/release").post(Map("queue" -> Seq("cars"), "csrfToken" -> Seq(token))))
//
//      // Then
//      response.status shouldBe OK
